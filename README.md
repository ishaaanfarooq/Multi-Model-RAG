...
[![CI](https://github.com/ishaaanfarooq/Multi-Model-RAG/actions/workflows/ci.yml/badge.svg)](https://github.com/ishaaanfarooq/Multi-Model-RAG/actions/workflows/ci.yml)
...
Every pull request and every push to `main` runs [the CI workflow](.github/workflows/ci.yml): the backend test suite and a frontend type-check.

```bash
make test       # full suite in the backend container (models available, nothing skipped)
make test-ci    # exactly what CI runs: clean Python 3.11, no ML runtime, fresh export
make help       # the other shortcuts (restart-backend, rebuild-frontend, eval, ...)
```

CI installs [`backend/requirements-ci.txt`](backend/requirements-ci.txt) — `requirements.txt` minus torch, FAISS, sentence-transformers and EasyOCR, generated by [`scripts/make_ci_requirements.py`](backend/scripts/make_ci_requirements.py) — so it takes minutes rather than an 11 GB image build. The handful of tests that need real models are marked `needs_ml`; in CI they are skipped with the reason printed, and they run in the backend container.
...
- [`backend/eval/VERIFIER.md`](backend/eval/VERIFIER.md) — is the verification module itself right? Answers with known groundedness (invented sentences, swapped numbers, wrong-document answers), what the verifier missed, and the unsupported-specifics check that closed the gap.
